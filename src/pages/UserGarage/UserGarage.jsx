...
export const UserGarage = () => {
  ...
  const carSpecHandler = () => {
    ...
    setUpdateShow(false);
    setError(null);
    setRegisterCarSpecData({}), setUpdateCarSpecData({});
  };
  ...
  const prevPageHandler = () => {
    ...
    if (paginationData.carPage <= 1) {
      ...
      getClientProfile(token, id, { ...paginationData, carPage: carPage }).then(
        (res) => {
          setProfileData(res);
          // Actualizar paginationData con los valores convertidos
          setPaginationData((prevState) => ({
  ...
  const nextPageHandler = () => {
    setRegisterCarSpecData({}), setUpdateCarSpecData({});
    if (
      paginationData.carLimit * paginationData.carPage >=
      itemsCount.carCount
    ) {
      null;
    } else {
  ...
  const buttonHandlerSaveCarSpecs = () => {
    ...
    if (validateCarSpecData(updateCarSpecData) === "empty object") {
      ...
      validateCarSpecData(updateCarSpecData) === "only empty strings"
    ) {
      setError("Ups! You didn't update anything!");
      return;
    } else if (
      keyValidator(updateCarSpecData, Object.keys(updateCarSpecData)) === null
    ) {
      setError("Ups! You finally didn't update anything!");
    } else if (
      validateCarSpecData(updateCarSpecData) === "some empty strings"
    ) {
      const updateData = {
        car_aero:
          updateCarSpecData.car_aero ||
          profileData.user?.car[0]?.carSpec?.car_aero,
        car_engine:
          updateCarSpecData.car_engine ||
          profileData.user?.car[0]?.carSpec?.car_engine,
        car_suspension:
          updateCarSpecData.car_suspension ||
          profileData.user?.car[0]?.carSpec?.car_suspension,
        car_tires:
          updateCarSpecData.car_tires ||
          profileData.user?.car[0]?.carSpec?.car_tires,
        car_differential:
          updateCarSpecData.car_differential ||
          profileData.user?.car[0]?.carSpec?.car_differential,
      };
      updateCarSpec(token, carId, updateData)
        .then(() => {
          getClientProfile(token, id, paginationData).then((res) => {
            setProfileData(res);
            setPaginationData((prevState) => ({
      ...
      updateCarSpec(token, carId, updateCarSpecData)
        .then(() => {
          getClientProfile(token, id, paginationData).then((res) => {
            setProfileData(res);
            setPaginationData((prevState) => ({
  ...
  const buttonHandlerRegisterCarSpecs = () => {
    ...
    const carId = profileData.user?.car[0]?.id;
    //comprobacion de que el objeto con los datos de registro no está vacio
    if (
      keyValidator(registerCarSpecData, Object.keys(registerCarSpecData)) ===
      null
    ) {
      setError("Ups! You finally didn't keep anything!");
      //comprobación de que el objeto con los datos de registro tiene las claves necesarias del formulario
    } else if (
      Object.keys(
        keyValidator(registerCarSpecData, Object.keys(registerCarSpecData))
      ).length < 5
      ...
      registerCarSpec(token, carId, registerCarSpecData)
        .then(() => {
          getClientProfile(token, id, paginationData).then((res) => {
            setProfileData(res);
            setPaginationData((prevState) => ({
  ...
  const buttonHandlerSaveNewcar = () => {
    setError(null);
    if (
      keyValidator(registerNewCarData, Object.keys(registerNewCarData)) === null
    ) {
      setError("Ups! You finally didn't keep anything!");
    } else if (
      Object.keys(
        keyValidator(registerNewCarData, Object.keys(registerNewCarData))
      ).length < 5
    ) {
      setError("Ups! You need to complete all fields!");
    } else {
  ...
  const buttonHandlerDeleteCar = () => {
    setError(null);
    const carId = profileData.user?.car[0]?.id;
    const carPage = paginationData.carPage - 1;
    deleteUserCar(token, carId)
      .then(() => {
        setGarageEditable(false);
        getClientProfile(token, id, { ...paginationData, carPage: carPage}).then((res) => {
          setProfileData(res);//aqui no llega
          setPaginationData((prevState) => ({
            ...prevState,
            carPage: Number(res.carPage),
            carLimit: Number(res.carLimit),
    ...
    <div className="profileData">
      <Card className="profileCard">
        <div className="imageWrapper">
          <Button className="imageButton" variant="secondary">
            <svg
          ...
          <Card.Img
            className="profileImage"
            variant="top"
            src="https://d1gl66oyi6i593.cloudfront.net/wp-content/uploads/2023/11/the-stig-identidad-secreto-top-gear-2.jpg"
          />
        </div>
        <Card.Body>
      ...
      <div className="profileDetails">
        {newCarEditable === true && garageEditable === false ? (
          <Button
            className="newCarButton"
            variant="danger"
            href="#newCarForm"
            onClick={() => newCarHandler()}
          >
            Maybe Next Season
          </Button>
        ) : null }
        {newCarEditable === false && garageEditable === true ? (
          <Button
            className="newCarButton"
            variant="primary"
            href="#newCarForm"
        ...
        {newCarEditable === true ? (
          <Card className="carSpecEdit" id="newCarForm">
            ...
            {JSON.stringify(registerNewCarData) !== "{}" ? (
              <Button
                variant="success"
                onClick={() => buttonHandlerSaveNewcar()}
              >
                Save New Car
              </Button>
            ) : null}
        ...
        {garageEditable === true && profileData.userCarsCount > 0 ? (
          <Card className="garageEdit" id="garageEdit">
            {profileData.userCarsCount > 1 ? (
            ...
            <div className="carCard">
              ...
              <div className="carDetails">
                <Card.Title className="garageCardModel">
                  {profileData.user?.car[0]?.car_brand?.toUpperCase()}{" "}
                  {profileData.user?.car[0]?.car_model?.toUpperCase()}
                </Card.Title>
                <Card.Text className="garageCardModel">
                  {profileData.user?.car[0]?.car_spec?.toUpperCase()}{" "}
                  {profileData.user?.car[0]?.car_category?.toUpperCase()}
                </Card.Text>
                <CardBody className="carSpec">
                  <Button
                    className="carSpecButton"
                    variant="dark"
                    href="#carSpecEditForm"
                    onClick={() => carSpecHandler(profileData.user?.car[0]?.id)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                  ...
                  <Button
                    variant="danger"
                    onClick={() => buttonHandlerDeleteCar()}
                  >
                    Delete Car
                  </Button>
                  {updateShow === true ? (
            ...
            profileData.user?.car[0]?.carSpec !== null ? (
              <Card className="carSpecEdit" id="carSpecEditForm">
                <CustomInput
                  placeholder={
                    profileData.user?.car[0]?.carSpec?.car_aero.toUpperCase() +
                      " AERO" || "AERO"
                  }
                  type={"text"}
                  name={"car_aero"}
                ...
                <CustomInput
                  placeholder={
                    profileData.user?.car[0]?.carSpec?.car_engine.toUpperCase() +
                      " ENGINE" || "ENGINE"
                  }
                  type={"text"}
                  name={"car_engine"}
                ...
                <CustomInput
                  placeholder={
                    profileData.user?.car[0]?.carSpec?.car_suspension.toUpperCase() +
                ...
                <CustomInput
                  placeholder={
                    profileData.user?.car[0]?.carSpec?.car_tires.toUpperCase() +
                      " TIRES" || "TIRES"
                  }
                  type={"text"}
                  name={"car_tires"}
                ...
                <CustomInput
                  placeholder={
                    profileData.user?.car[0]?.carSpec?.car_differential.toUpperCase() +
                ...
                {JSON.stringify(updateCarSpecData) !== "{}" ? (
                  <Button
                    variant="success"
                    onClick={() => buttonHandlerSaveCarSpecs()}
                  >
                    Update Specs
                  </Button>
                ) : null}
            ...
            profileData.user?.car[0]?.carSpec === null ? (
              <Card className="carSpecEdit" id="carSpecEditForm">
                ...
                {JSON.stringify(registerCarSpecData) !== "{}" ? (
                  <Button
                    variant="success"
                    onClick={() => buttonHandlerRegisterCarSpecs()}
                  >
                    Save Your Specs
                  </Button>
                ) : null}
